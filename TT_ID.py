...
def Dijkstra_heuristic(state, actual_depth):
        TMP_board = HexBoard(size)
        ...
        return(((min_distance_blue, actual_depth), (-min_distance_red, actual_depth)))
...
def eval(state, actual_depth):
    ...
    if TMP_board.check_win(2) or TMP_board.check_win(1) or sum([TMP_board.is_empty(x) for x in TMP_board.board]) == 0:
        if TMP_board.check_win(2):
            return((100, actual_depth), (100, actual_depth))
        elif TMP_board.check_win(1):
            return((-100, actual_depth), (-100, actual_depth))
        else:
            return((0, actual_depth),  (0, actual_depth))
    else:
        value = Dijkstra_heuristic(state, actual_depth)
        return(value)
...
def minimax(state, d, actual_depth, start, alpha = -Inf, beta = Inf): # Returns the value of the LEAF following optimal play for both players 
    TMP_board = HexBoard(size)
    TMP_board.board = copy.deepcopy(state)
    end = time.time()
    best_move_tt = [None]
    ...
    if  keyboard.is_pressed("p") or (end - start) >= think_time:
        return(eval(state, actual_depth))
    if d <= 0:
        return(eval(state, actual_depth))
    else:
        if sum([TMP_board.is_color(x, 1) for x in TMP_board.board]) >= \
            ...
            value = ((-Inf, actual_depth), (-Inf, actual_depth))  # MAX PLAYER/NODE
            if TMP_board.check_win(2) or TMP_board.check_win(1) or sum([TMP_board.is_empty(x) for x in TMP_board.board]) == 0: # IF IT IS A LEAF
                ...
                for action in best_move_tt + getMoveList(state): # Assessing a MAX NODE
                    if action is None:
                        continue
                    value_c = minimax(result(state, action), d-1, actual_depth + 1,  start, alpha, beta) 
                    if value_c[1][0] > value[0][0]:
                        bm = action
                        value = value_c
                    alpha = max(value[1][0], alpha)
            ...
            value = ((Inf, actual_depth), (Inf, actual_depth)) # MIN PLAYER/NODE
            if TMP_board.check_win(2) or TMP_board.check_win(1) or sum([TMP_board.is_empty(x) for x in TMP_board.board]) == 0: # IF IT IS A LEAF
                return(eval(state, actual_depth))   
            else:
                for action in best_move_tt + getMoveList(state):
                    if action is None:
                        continue
                    value_c = minimax(result(state, action), d-1, actual_depth + 1, start, alpha, beta) 
                    if value_c[0][0] < value[0][0]:
                        bm = action
                        value = value_c
                    beta = min(value[0][0], beta)
    ...
    while not keyboard.is_pressed("p") and (end - start) < think_time:
        ...
        if color == 2:
            for action in best_move_tt + getMoveList(state):
                ...
                U = minimax(result(state, action), d = d, actual_depth = 1, start = start)
                print(U)

                if U[1][0] > Best_outcome[1][0] and U[1][1] > Best_outcome[1][1]:
                    Best_outcome = U
                    Best_move = action
            ...
            for action in best_move_tt + getMoveList(state):
                ...
                if U[0][0] < Best_outcome[0][0] and U[0][1] > Best_outcome[0][1]:
                    Best_outcome = U
                    Best_move = action
            moves.append((Best_move, color))
