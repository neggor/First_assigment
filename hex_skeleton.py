class HexBoard:
  ...
  def __init__(self, board_size):
    self.board = {} ## A dictionary
    self.size = board_size
    self.game_over = False
    for x in range(board_size):
      for y in range (board_size):
        self.board[x,y] = HexBoard.EMPTY ## Allocate status of position by a coordinates key
  ...
  def place(self, coordinates, color): # To place a stone in the board
  ...
  def border(self, color, move): ## Check if the stone is actually in the border
    (nx, ny) = move
    return (color == HexBoard.BLUE and nx == self.size-1) or (color == HexBoard.RED and ny == self.size-1)

  def traverse(self, color, move, visited): ## Visited must be a dictionary 
    if not self.is_color(move, color) or (move in visited and visited[move]): return False # Not occupied by my color or not visited
    if self.border(color, move): return True # We reached the border
    visited[move] = True
    for n in self.get_neighbors(move):
      if self.traverse(color, n, visited): return True ## Calls itself till find the border
    return False

  def check_win(self, color):
    for i in range(self.size):
      if color == HexBoard.BLUE: move = (0,i) ## For possible stones in win possition check the whole queue
      else: move = (i,0)
      if self.traverse(color, move, {}): # If queue form one side to another then win
        return True
    return False
